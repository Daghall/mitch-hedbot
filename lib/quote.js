...
exports.handler = async function (event) {
  debug("Quoting...");
  const { type, push, dev, index, extended } = event.queryStringParameters ?? {};
  const quote = getQuote(type, index, extended);
...
function getQuote(type, index, extended) {
  if (!index) {
    switch (type) {
      case "week":
        index = getWeekNumber();
